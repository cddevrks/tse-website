...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Only select anchor links inside the nav so dropdown-toggle spans aren't included
  ...
  dropdowns.forEach((dropdown) => {
    ...
    const chevronIcon = dropdownToggle
      ? dropdownToggle.querySelector("i")
      : null;

    if (dropdownToggle && dropdownMenu) {
      // Ensure accessibility attributes for toggle spans
      ...
      dropdownToggle.addEventListener("click", (e) => {
        if (window.innerWidth <= 768) {
          ...
          // Close all other dropdowns and reset their icons/aria
          dropdowns.forEach((otherDropdown) => {
            if (otherDropdown !== dropdown) {
              const otherMenu = otherDropdown.querySelector(".dropdown-menu");
              const otherIcon =
                otherDropdown.querySelector(".dropdown-toggle i");
              const otherToggle =
  ...
  (function () {
    ...
    const captionEl = lightbox
      ? lightbox.querySelector(".lightbox-caption")
      : null;
    const counterEl = lightbox
      ? lightbox.querySelector(".lightbox-counter")
      : null;
    const thumbsContainer = lightbox
      ? lightbox.querySelector(".lightbox-thumbnails")
      : null;
    let currentIndex = 0;
    ...
    function buildVisibleItems() {
      const visible = galleryFigures.filter(
        (fig) => getComputedStyle(fig).display !== "none",
      );
      return visible.map((fig, idx) => {
        const img = fig.querySelector("img");
        const caption = fig.querySelector(".gallery-caption")
          ? fig.querySelector(".gallery-caption").textContent.trim()
          : img.alt || "";
    ...
    function updateLightboxUI(items) {
      ...
      if (thumbsContainer) {
        ...
        const activeThumb = thumbsContainer.querySelector(
    ...
    function closeLightbox() {
      ...
      // restore focus
      if (focusableBefore && typeof focusableBefore.focus === "function")
    ...
    galleryFigures.forEach((fig) => {
      ...
      fig.setAttribute(
        "aria-label",
        fig.querySelector(".gallery-caption")
          ? fig.querySelector(".gallery-caption").textContent.trim()
          : `Open image`,
      );
  ...
  // Handle image loading for lazy-loaded images
